(function ($, undefined) {
	...
	Datepicker.prototype = {
		...
		_trigger: function (event, altdate) {
			let date = altdate || this.dates.get(-1),
		...
		fill: function () {
			...
			while (prevMonth.valueOf() < nextMonth) {
				...
				if (this.o.beforeShowDay !== $.noop) {
					let before = this.o.beforeShowDay(this._utc_to_local(prevMonth));
					if (before === undefined)
						before = {};
			...
			let yearCont = this.picker.find('.datepicker-years')
				...
				.find('td');
			year -= 1;
			let years = $.map(this.dates, function (d) {
					...
					case 'th':
						switch (target[0].className) {
							...
							case 'today':
								...
								this.showMode(-2);
								let which = this.o.todayBtn === 'linked' ? null : 'view';
		...
		_toggle_multidate: function (date) {
			let ix = this.dates.contains(date);
			if (!date) {
				this.dates.clear();
			}
			else if (ix !== -1) {
	...
	let DateRangePicker = function (element, options) {
		this.element = $(element);
	...
	DateRangePicker.prototype = {
		...
		updateRanges: function () {
			let range = $.map(this.dates, function (d) {
				return d.valueOf();
			});
	...
	$.fn.datepicker = function (option) {
		...
		this.each(function () {
			...
			if (!data) {
				let elopts = opts_from_el(this, 'date'),
					// Preliminary otions
	...
	let defaults = $.fn.datepicker.defaults = {
		autoclose: false,
		beforeShowDay: $.noop,
		calendarWeeks: false,
		clearBtn: false,
	...
	let locale_opts = $.fn.datepicker.locale_opts = [
		'format',
		'rtl',
		'weekStart'
	];
	$.fn.datepicker.Constructor = Datepicker;
	let dates = $.fn.datepicker.dates = {
		en: {
